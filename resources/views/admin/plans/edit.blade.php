...
@section('subtitle', "Update {$plan->name} plan")
...
@section('content')
    ...
    <div class="flex items-center gap-3 mb-6 animate-fade-up">
        <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-brand-50 to-indigo-50 flex items-center justify-center border border-brand-100/50">
            <svg class="w-5 h-5 text-brand-600" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/></svg>
        </div>
        <div>
            <h1 class="text-2xl font-extrabold text-slate-800">Edit {{ $plan->name }}</h1>
            <p class="text-sm text-slate-400">Update plan details and pricing</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.plans.update', $plan) }}">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="2"
                            class="w-full px-4 py-2.5 bg-gray-50/80 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/10 transition resize-none">{{ old('description', $plan->description) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 pt-4 pb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="price_monthly" name="price_monthly" value="{{ old('price_monthly', $plan->price_monthly) }}" step="0.01" min="0"
                    ...
                    <div>
                        ...
                        <input type="number" id="price_yearly" name="price_yearly" value="{{ old('price_yearly', $plan->price_yearly) }}" step="0.01" min="0"
            ...
            <div class="px-6 pt-4 pb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="max_students" name="max_students" value="{{ old('max_students', $plan->max_students) }}" min="0" placeholder="Unlimited"
                    ...
                    <div>
                        ...
                        <input type="number" id="max_staff" name="max_staff" value="{{ old('max_staff', $plan->max_staff) }}" min="0" placeholder="Unlimited"
                            class="w-full px-4 py-2.5 bg-gray-50/80 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/10 transition">
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 pt-4 pb-4">
                ...
                <textarea id="features" name="features" rows="6"
                    class="w-full px-4 py-2.5 bg-gray-50/80 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/10 transition resize-none font-mono">{{ old('features', is_array($plan->features) ? implode("\n", $plan->features) : '') }}</textarea>
            </div>
