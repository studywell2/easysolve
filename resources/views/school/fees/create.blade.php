...
@section('content')
    <div class="animate-fade-up">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-extrabold text-slate-900">Add Fee</h1>
            <p class="text-sm text-slate-400 mt-0.5">Create a new fee structure</p>
        </div>
            ...
            <form method="POST" action="{{ route('school.fees.store') }}">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="e.g. Tuition Fee" class="w-full px-4 py-2.5 bg-gray-50/80 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/10 transition">
                        @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label for="amount" class="block text-sm font-semibold text-slate-700 mb-1.5">Amount (₦)</label>
                        <input type="number" id="amount" name="amount" min="0" step="0.01" value="{{ old('amount') }}" class="w-full px-4 py-2.5 bg-gray-50/80 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/10 transition">
                        @error('amount')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="class_id" class="block text-sm font-semibold text-slate-700 mb-1.5">Class (optional)</label>
                        ...
                        <div>
                            <label for="term_id" class="block text-sm font-semibold text-slate-700 mb-1.5">Term (optional)</label>
